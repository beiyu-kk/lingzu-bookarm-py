...
pip install -e ".[gui]"       # 含 MotorStudio GUI 支持
```

基础安装已包含 `pyrealsense2`、`open3d`、`opencv-python`、`pyserial` 等当前 SDK 常用依赖。`camera` / `realsense` extras 保留为空的兼容入口；GUI 新环境优先使用 `.[gui]`，`.[debugger]` 是兼容别名。
...
| 参数 | 说明 |
|------|------|
| `gripper_angle` | 目标位置 (rad)。当前应用约定 `0.0` 附近为打开，约 `1.57` rad 为关闭 |
...
# 夹爪控制：0.0 rad 附近打开，约 1.57 rad 关闭
