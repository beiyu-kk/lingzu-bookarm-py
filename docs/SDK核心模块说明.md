...
当前 MotorStudio 点云页默认使用 `color=1280x720@30`、`depth=1280x720@30`，这是 D435 类设备常见的同步 RGB-D 配置。`RealSenseD435` 在启动时如果遇到不支持的 RGB-D 流组合，会自动尝试常见降级模式（如 `1280x720`、`848x480`、`640x480`），避免固定在 `1920x1080` 彩色流导致 pipeline resolve 失败。
...
`demo/xbox_control.py` 使用这些 profile 做笛卡尔遥操作。当前默认映射中，D-pad 上为夹爪小步打开，D-pad 下为夹爪小步关闭；单击会移动一个小增量，按住会按连续速度平滑累积夹爪目标角度。
