...
def _default_mesh_dir() -> Path:
    local_meshes = _get_base_path() / "resources" / "meshes"
    if local_meshes.exists():
        return local_meshes

    ros_candidates = [
        _get_base_path().parent / "el_a3_ros" / "el_a3_description" / "meshes",
        _get_base_path().parent.parent.parent / "el_a3_ros" / "el_a3_description" / "meshes",
    ]
...
class Viewer3DPanel(QWidget):
    ...
    def __init__(self, parent=None, urdf_path=None, mesh_dir=None):
        ...
        self._tcp_line_actor = None
        self._tcp_line_mesh = None
    ...
    def initialize_model(self):
        ...
        try:
            ...
            self._update_tcp_offset_visual(transforms, render=False)
    ...
    def _add_tcp_marker(self):
        """添加 TCP 点和 end_effector -> TCP 偏移连线。"""
        if not self._plotter:
            return
        try:
            if self._tcp_actor is not None:
                return
            self._tcp_line_mesh = pv.Line(
                pointa=(0.0, 0.0, 0.0),
                pointb=(0.0, 0.0, 0.0),
                resolution=1,
            )
            self._tcp_line_actor = self._plotter.add_mesh(
                self._tcp_line_mesh,
                color=(1.0, 0.78, 0.10),
                line_width=4,
                name="tcp_offset_line",
            )
            self._tcp_line_actor.SetVisibility(False)
    ...
    def set_tcp_offset(self, tcp_offset):
        ...
        for idx in range(min(6, len(source))):
            values[idx] = float(source[idx])
        self._tcp_offset = values
        self._refresh_tcp_visual()

    def _refresh_tcp_visual(self):
        if not self._initialized or self._model is None:
            return
        transforms = self._model.compute_link_transforms(self._current_angles)
        self._update_tcp_offset_visual(transforms, render=True)

    def _tcp_offset_transform(self) -> np.ndarray:
        return _make_transform(
            np.asarray(self._tcp_offset[:3], dtype=float),
            np.asarray(self._tcp_offset[3:6], dtype=float),
        )

    @staticmethod
    def _end_effector_transform(transforms: Dict[str, np.ndarray]) -> Optional[np.ndarray]:
        for link_name in ("end_effector", "gripper_link", "l5_l6_urdf_asm"):
            transform = transforms.get(link_name)
            if transform is not None:
                return transform
        return None

    def _update_tcp_offset_visual(
        self,
        transforms: Optional[Dict[str, np.ndarray]] = None,
        *,
        render: bool,
    ):
        if not self._plotter or self._tcp_actor is None or self._model is None:
            return
        if transforms is None:
            transforms = self._model.compute_link_transforms(self._current_angles)
        ee_T = self._end_effector_transform(transforms)
        if ee_T is None:
            return

        tcp_T = ee_T @ self._tcp_offset_transform()
        ee_pos = np.asarray(ee_T[:3, 3], dtype=float)
        tcp_pos = np.asarray(tcp_T[:3, 3], dtype=float)
        self._tcp_actor.SetPosition(float(tcp_pos[0]), float(tcp_pos[1]), float(tcp_pos[2]))
        self._tcp_actor.SetVisibility(True)
        self._tcp_visible = True

        if self._tcp_line_mesh is not None and self._tcp_line_actor is not None:
            self._tcp_line_mesh.points = np.vstack([ee_pos, tcp_pos])
            has_visible_offset = bool(np.linalg.norm(tcp_pos - ee_pos) > 1e-6)
            self._tcp_line_actor.SetVisibility(has_visible_offset)
    ...
    def update_tcp_point(self, end_pose):
        """更新 TCP 标记；优先使用 viewer 内部 TCP offset 可视化。"""
