...
class JointControlPanel(QWidget):
    ...
    go_zero_requested = pyqtSignal(list, float)  # 6 个弧度值 + 运动时间（s）
    ...
    def __init__(self, parent=None):
        ...
        self._header_labels = []
        self._name_labels = []
        self._zero_duration_label = None
        self._init_ui()
    ...
    def _init_ui(self):
        ...
        zero_group = QHBoxLayout()
        zero_group.setContentsMargins(0, 0, 0, 0)
        zero_group.setSpacing(6)

        self.zero_btn = QPushButton(tr("jc.go_zero"))
        self.zero_btn.setFixedSize(64, 28)
        self.zero_btn.clicked.connect(self._on_go_zero)
        zero_group.addWidget(self.zero_btn)

        self._zero_duration_label = QLabel(tr("jc.zero_duration"))
        self._zero_duration_label.setAlignment(Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignRight)
        self._zero_duration_label.setFixedHeight(28)
        zero_group.addWidget(self._zero_duration_label)

        self.zero_duration_spin = NoWheelDoubleSpinBox()
        self.zero_duration_spin.setRange(0.5, 30.0)
        self.zero_duration_spin.setDecimals(2)
        self.zero_duration_spin.setSingleStep(0.1)
        self.zero_duration_spin.setValue(2.5)
        self.zero_duration_spin.setSuffix(" s")
        self.zero_duration_spin.setFocusPolicy(Qt.FocusPolicy.ClickFocus)
        self.zero_duration_spin.setFixedWidth(78)
        self.zero_duration_spin.setFixedHeight(28)
        zero_group.addWidget(self.zero_duration_spin)
        btn_layout.addLayout(zero_group)
    ...
    def retranslate_ui(self):
        ...
        if self._zero_duration_label is not None:
            self._zero_duration_label.setText(tr("jc.zero_duration"))
    ...
    def _on_go_zero(self):
        ...
        self._updating_from_feedback = False
        self.go_zero_requested.emit(positions_rad, float(self.zero_duration_spin.value()))
